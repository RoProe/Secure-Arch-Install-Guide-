...
#
# This module is bundled into the UKI initramfs by dracut.
# It runs AFTER LUKS is unlocked but BEFORE root is mounted.
# The user sees a simple menu:
#   [Enter] → normal boot (5 second timeout, auto-selects this)
#   [s]     → numbered list of Snapper snapshots → select one → boot it
#
# When a snapshot is selected, the initramfs mounts that snapshot's subvolume
# (@snapshots/N/snapshot) as root instead of @, giving you a clean rollback.
#
...
install() {
    ...
    inst_multiple btrfs awk grep sed cat find
}
EOF
...
# Mount the raw Btrfs volume (subvolid=5 = top-level, all subvolumes visible)
mkdir -p "\$BTRFS_MNT"
...
while IFS= read -r info; do
    num=\$(echo "\$info" | grep -oE '@snapshots/[0-9]+' | grep -oE '[0-9]+$')
    [ -z "\$num" ] && continue
    snap_subvol="\$BTRFS_MNT/@snapshots/\${num}/snapshot"
    [ -d "\$snap_subvol" ] || continue
    desc=\$(grep -oP '(?<=<description>)[^<]+' "\$info" 2>/dev/null || echo "—")
    date=\$(grep -oP '(?<=<date>)[^<]+' "\$info" 2>/dev/null | awk '{print \$1}' || echo "")
    stype=\$(grep -oP '(?<=<type>)[^<]+' "\$info" 2>/dev/null || echo "")
    SNAP_IDS+=("\$num")
    SNAP_LABELS+=("\$date [\$stype] \$desc")
done < <(find "\$BTRFS_MNT/@snapshots" -name "info.xml" 2>/dev/null \
...
echo ""
read -rp "  Enter snapshot number: " CHOICE
...
# Write the chosen subvolume path to a file dracut's mount hook will read.
# We override rootflags to mount the snapshot subvolume instead of @.
SNAP_SUBVOL="@snapshots/\${CHOICE}/snapshot"
echo "Booting snapshot \${CHOICE}: \${SNAP_SUBVOL}"
# Patch the root mount flags for this boot only
echo "rw,noatime,compress=zstd,subvol=\${SNAP_SUBVOL}" \
    > /run/initramfs/rootflags-override
MENUEOF
